...
class MyDataset(Dataset):
    def __init__(self, img_dir, mask_dir, transforms = True):
    ...
    def __getitem__(self, index):
        img_name = self.img_dir + '/Data' + str(index + 1) + '.tif'
        mask_name = self.mask_dir + '/Data' + str(index + 1) + '_reference.tif'
        image = cv2.imread(img_name, -1) #保留位深16
        # image = self.read_tif(img_name)
        mask = cv2.imread(mask_name, -1)

        sample = {'img' : image, 'label' : mask}
        if self.transforms:
            sample = self.transforms_tr(sample)
        image, mask = sample['img'], sample['label']
...
if __name__ == "__main__":
    data = MyDataset('data/img', 'data/mask')
